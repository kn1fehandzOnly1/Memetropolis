...
export default function ProfileModal({ isOpen, onClose }) {
  const { user, posts, setActiveModal } = useStore();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#121219] border border-slate-800 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col">
        ...
        <div className="h-24 bg-gradient-to-r from-cyan-600 via-indigo-600 to-purple-700 relative p-4 flex items-start justify-between">
          ...
          <button onClick={onClose} className="text-white/80 hover:text-white p-1.5 rounded-xl bg-black/50 backdrop-blur-md transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="px-6 pb-4 pt-2 relative flex-1 overflow-y-auto">
            ...
            <div className="flex items-end space-x-4">
              <div className="relative shrink-0">
                <img 
                  src={user.avatar} 
                  alt={user.username} 
                  className={`w-24 h-24 rounded-full object-cover ring-4 ${user.isProPlus ? 'ring-pink-500 shadow-xl shadow-pink-500/30' : user.isPro ? 'ring-amber-400 shadow-xl shadow-amber-400/20' : 'ring-slate-800'}`}
                />
                {(user.isPro || user.isProPlus) && (
                  <div className={`absolute bottom-0 right-0 p-1.5 rounded-full text-[10px] font-black ${user.isProPlus ? 'badge-pro-plus' : 'badge-pro'}`}>
                    <Crown size={14} />
                  </div>
                )}
              </div>
              ...
              <div className="pt-2 space-y-1">
                <div className="flex items-center space-x-2 flex-wrap gap-1">
                  <h2 className="font-black text-2xl text-white font-outfit leading-tight">{user.username}</h2>
                  {user.isProPlus && <span className="badge-pro-plus px-2.5 py-0.5 rounded-full text-[10px]">PRO+</span>}
                  {user.isPro && !user.isProPlus && <span className="badge-pro px-2.5 py-0.5 rounded-full text-[10px]">PRO</span>}
                </div>
                <p className="text-xs text-slate-300 leading-normal font-medium max-w-sm">{user.bio}</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 shrink-0 pt-2 sm:pt-0">
              {!user.isPro && (
                <button
                  ...
                  className="bg-gradient-to-r from-amber-400 to-orange-500 hover:opacity-90 text-black font-extrabold px-4 py-2 rounded-full text-xs flex items-center space-x-1.5 shadow-lg shadow-amber-500/10 transition-transform hover:scale-105"
                >
                  <Crown size={15} />
              ...
              <button
                ...
                className="bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 font-bold px-3.5 py-2 rounded-full text-xs flex items-center space-x-1.5 transition-all hover:scale-105"
              >
                <Coins size={15} />
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-900/30 via-[#181824] to-indigo-900/30 border border-purple-500/30 mb-5 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <h4 className="font-extrabold text-sm text-white">Creator Milestone Rewards</h4>
              </div>
              <span className="text-xs font-black text-amber-400">+{nextMilestone.rewardCoins} COINS BONUS</span>
            </div>

            <p className="text-xs text-slate-300">
              Reach <span className="font-bold text-white">{nextMilestone.count} subscribers</span> to unlock <span className="text-cyan-400 font-bold">{nextMilestone.title}</span> and get <span className="text-amber-400 font-extrabold">+{nextMilestone.rewardCoins} Free Coins</span>!
            </p>
          ...
          <div className="flex border-b border-slate-800 mb-4">
            ...
            <button
              ...
              className={`px-4 py-2.5 font-extrabold text-xs border-b-2 transition-all flex items-center space-x-1.5 ${
                activeTab === 'subscriptions' ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-slate-400 hover:text-white'
              }`}
            >
              <Users size={14} />
            ...
            <button
              ...
              className={`px-4 py-2.5 font-extrabold text-xs border-b-2 transition-all flex items-center space-x-1.5 ${
                activeTab === 'saved' ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-slate-400 hover:text-white'
              }`}
            >
              <Bookmark size={14} />
          ...
          {activeTab === 'overview' && (
            <div className="space-y-3">
              {myPosts.length === 0 ? (
                <p className="text-slate-500 text-xs text-center py-6">You haven’t published any memes yet.</p>
              ) : (
          ...
          {activeTab === 'subscriptions' && (
            <div className="space-y-2">
              {(!user.following || user.following.length === 0) ? (
                <p className="text-slate-500 text-xs text-center py-6">You are not subscribed to any creators.</p>
              ) : (
                  ...
                  <div key={username} className="flex items-center justify-between p-3 rounded-xl bg-[#171722] border border-slate-800">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">Creator • ViralDrop</div>
                      </div>
                    </div>
          ...
          {activeTab === 'saved' && (
            <div className="space-y-3">
              {savedPosts.length === 0 ? (
                <p className="text-slate-500 text-xs text-center py-6">No saved memes.</p>
              ) : (
                <div className="grid grid-cols-2 gap-3">
                  {savedPosts.map(post => (
                    <div key={post.id} className="rounded-xl overflow-hidden bg-[#171722] border border-slate-800 p-2">
